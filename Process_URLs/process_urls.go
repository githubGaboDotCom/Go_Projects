...
func getPerson(wg *sync.WaitGroup, urls []string) {
...
func getSpecies(wg *sync.WaitGroup, urls_Species []string) {

	for _, url := range urls_Species {
		...
		fmt.Println("-----------------------------------------------")
		fmt.Println("Name           : ", person.Name)
		fmt.Println("Average_height : ", person.Average_height)
		fmt.Println("Language       : ", person.Language)
	}
...
func main() {
	...
	go getPerson(&wg, urls)
	go getSpecies(&wg, urls_Species)
	wg.Add(2)
